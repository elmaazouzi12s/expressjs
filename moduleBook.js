...
module.exports.GetLivre = (id) => {
    const livre = this.livres.filter((livre) => livre.id == id)
    if (livre.length === 0) {
        return -1
    } else {
        return livre[0]
...
module.exports.UpdateLivre = (livre) => {
...
module.exports.DeleteLivre = (id) => {
    if (this.GetLivre(id) === -1) {
        return 0
    } else {
        this.livres = livres.filter((item) => item.id !== parseInt(id))
        return 1;
    }
}